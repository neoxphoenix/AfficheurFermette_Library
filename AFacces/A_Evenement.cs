...
namespace Projet_AFFICHEURFERMETTE.MDF.Acces
{
    ...
    public class A_Evenement : ADBase
    {
        ...
        public int Ajouter(DateTime DateDebut, DateTime DateFin, string Description, int TypeEvenement, int IDtitre, int IDlieu)
        {
            ...
            Commande.Parameters.AddWithValue("@IDtitre", IDtitre);
            Commande.Parameters.AddWithValue("@IDlieu", IDlieu);
        ...
        public List<C_Evenement> Lire(string Index)
        {
            ...
            while (dr.Read())
            {
                ...
                tmp.IDtitre = int.Parse(dr["IDtitre"].ToString());
                tmp.IDlieu = int.Parse(dr["IDlieu"].ToString());
                res.Add(tmp);
            }
            dr.Close();
            Commande.Connection.Close();
        ...
        public C_Evenement Lire_ID(int ID)
        {
            ...
            while (dr.Read())
            {
                ...
                res.IDtitre = int.Parse(dr["IDtitre"].ToString());
                res.IDlieu = int.Parse(dr["IDlieu"].ToString());
            }
            dr.Close();
            Commande.Connection.Close();
            return res;
